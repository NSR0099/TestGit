...
const CitizenDashboard = () => {
    ...
    const fetchIncidents = async () => {
        try {
            ...
            const response = await fetch(`${BASE_URL}/incidents?${queryParams.toString()}`);
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                const BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api';
                const response = await fetch(`${BASE_URL}/incidents/analyze`, {
                    method: 'POST',
    ...
    const handleUpvote = async (incidentId) => {
        try {
            const BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api';
            const response = await fetch(`${BASE_URL}/incidents/${incidentId}/upvote`, {
                method: 'PUT'
            });
                                                ...
                                                <div className="incident-body">
                                                    ...
                                                    {incident.media && (
                                                        <div className="incident-media">
                                                            <img
                                                                src={`${import.meta.env.VITE_API_BASE_URL ? import.meta.env.VITE_API_BASE_URL.replace('/api', '') : 'http://localhost:5000'}${incident.media}`}
